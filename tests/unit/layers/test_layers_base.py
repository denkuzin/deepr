...
@dpr.layers.layer(n_in=1, n_out=1, lazy=True)
def AddOneLazy():
    """AddOneLazy"""
    return AddOffset(offset=1)
...
def test_layers_call(session):
    ...
    offset_layer = AddOne(inputs="x", outputs="y")
...
def test_layers_decorator_from_constructor(session):
    """Test decorator on constructors."""
    # Check decorated function properties
    assert not issubclass(AddOne, dpr.layers.Layer)
...
def test_layers_decorator_from_lazy_constructor(session):
    """Test decorator on lazy constructors."""
    # Check decorated function properties
    assert issubclass(AddOneLazy, dpr.layers.Layer)
    assert AddOneLazy.__name__ == "AddOneLazy"
    assert AddOneLazy.__doc__ == "AddOneLazy"
    assert AddOneLazy.__module__ == __name__

    # Check instance properties
    offset_layer = AddOneLazy()
    assert isinstance(offset_layer, AddOneLazy)
    assert not isinstance(offset_layer, AddOffset)
    result = offset_layer.forward(tf.constant(1))
    assert session.run(result) == 2
...
def test_layers_decorator_from_lazy_constructor_laziness():
    """Test decorator laziness."""
...
def test_layers_decorator_from_constructor_incorrect_type():
    """Test decorator failure if constructor does not return a Layer."""
    with pytest.raises(TypeError):

        @dpr.layers.layer(n_in=1, n_out=1)
        def NotALayerConstructor():
            pass

        NotALayerConstructor()
